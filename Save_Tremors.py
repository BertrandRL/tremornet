...
def build_database_tremors(df,indices,network,station,channels,nfft,s_rate,l,dim1,dim2,dim3,path,latitude,longitude):
    ...
    for j in indices: #(len(times_cat)):
        try: 
            ...
            if len(tr)>0:
                with torch.no_grad():
                    ...
                    #print('shape: ', Fourier_trans.shape)
        ...
        if len(spectrograms)>0 and (i%100==0 or i==len(times_cat)):
            ...
            if os.path.exists(path+'/Database_BH_tremors_%s.hdf5'%station):
                with h5py.File(path+'/Database_BH_tremors_%s.hdf5'%station, 'a') as f:
                    f["tremors"].resize((f["tremors"].shape[0] + tremors.shape[0]), axis=0)    
                    f["tremors"][-tremors.shape[0]:] = tremors     
            else: 
                with h5py.File(path+'/Database_BH_tremors_%s.hdf5'%station, 'w') as f:
                    dsetX = f.create_dataset("tremors", data=tremors,maxshape=(None,None,None,None),chunks=True, dtype='float32')
            
            # Save times
            if os.path.exists(path+'/times_tremors_BH_%s.csv'%station):
                tt=pd.Series(np.array(times))
                tt.to_csv(path+'/times_tremors_BH_%s.csv'%station, mode='a', header=False, index=False)
            else: 
                tt=pd.Series(np.array(times))
                tt.to_csv(path+'/times_tremors_BH_%s.csv'%station, header=False, index=False)
            spectrograms=[]
            times=[]
...
path='/ram/mnt/local/projects/criticalstress-ldrd/bertrandrl/DB_tremor_300'

for station in station_list:
    
    inventory = client.get_stations(network=network,station=station,location='*',channel=channels.split(',')[0],starttime='2009-01-01',endtime='2021-05-01')
    ...
    try: 
        saved_events=pd.read_csv(path+'/times_tremors_BH_%s.csv'%station,header=None)
